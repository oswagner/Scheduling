...
public class CircularLinkedList<T> implements List<T> {
	...
	// clear list by getting rid of data in tail and setting pointer of tail to
	// itself
	public void clear() {
		tail.setData(null);
		tail.setNext(tail);
	...
	public T remove(int index) {
		T old = null;
		if (index == 0)
			old = removeFirst(getNode(index));
		else if (index == size() - 1)
			old = removeLast(getNode(index));
		else
			old = removeNode(getNode(index), getNode(index - 1));
	...
	// rotate the head of the list to the tail of the list, a counter-clock wise
	// rotate
	...
	private class LinkedListIterator implements Iterator<T> {
		...
		// constructor of linked list iterator, will set the current node to the first
		// node of the list and the previous will
